...
- TUI: key hints render non-bold across the top bar, the confirm popup, and the help overlay; the top bar gains an always-on `↑↓:scroll` chip (order: `pull · help · panes · scroll · theme · quit`) and now drops chips by priority as the terminal narrows (keeping `pull` / `help` longest, `scroll` first to go) instead of hiding the whole strip; and the right pane's tab strip dims to muted + first-letter-underlined (non-bold) when the pane is unfocused, matching the Models pane title.
