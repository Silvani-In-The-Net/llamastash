...
- **Backend-neutral substrate seams (MLX/vLLM-ready, no consumer yet).** Two reusable seams ship as backend-neutral substrate so a future safetensors/HF-format engine is a predicate + projection + knob table, not a rewrite — both empty/invisible for every shipping backend, proven generic by neutrality / stub tests (the first consumer, MLX, is a follow-up plan). (1) **Two-layer discovery:** `discovery::hf_repos` walks the same HF hub cache roots GGUF discovery scans and yields neutral `HfRepoCandidate` rows for non-GGUF repos (safetensors present, GGUF absent); a shared `config_to_metadata()` fills the generic `ModelMetadata` fields from `config.json` + `tokenizer_config.json`. `ModelMetadata` gains `quant_label: Option<String>` for non-GGML affine quant strings (GGUF leaves it `None` → GGUF output byte-stable; not surfaced in JSON until a backend sets it). The module references no backend symbols (pinned by a neutrality test). (2) **Per-backend native knobs:** `launch::native_knobs` is a string-id channel **parallel** to the llama.cpp `KnobField` IR (which stays llama.cpp-keyed). `Backend::native_knobs()` (default `&[]`) declares `NativeKnobDescriptor`s (`Cycle`/`FreeText`/`Bool`); the picker renders them below the typed knobs, values persist in `LaunchParams.backend_knobs` / `PresetBody.backend_knobs` (additive, `skip_serializing_if` empty → byte-stable), and `native_knobs::translate` renders set values to argv with the same forbidden-prefix strip `compose` applies to extras. Orthogonal to `capabilities()`. See `docs/plans/2026-06-24-001-feat-shared-safetensors-discovery-substrate-plan.md` (substrate) and `2026-06-24-002-feat-mlx-backend-plan.md` (the MLX consumer).
- **`llamastash pull`** graduated in v2 from the v1 `unimplemented!` shim. MVP shape: `llamastash pull <owner/repo[:filename.gguf]>` — downloads via the [`hf-hub`](https://crates.io/crates/hf-hub) crate (0.5 line, resolves the same `reqwest 0.12` we pin elsewhere) into the canonical HF cache layout that discovery already scans. A whole-repo `pull owner/repo` of a mixed-format repo prefers `*.safetensors` and drops the duplicate PyTorch `*.bin`/`*.pth` weights (configs/tokenizers kept); a no-op for GGUF / PyTorch-only repos, and the pinned `:filename.gguf` arm is unaffected. The TUI's `d` HuggingFace pull dialog is the interactive face of this primitive; the CLI `llamastash pull <slug>` stays the only non-TUI browse surface (the dialog is TUI-only, no HTTP / MCP equivalent in v2). The `cli/output.rs::list_json` / `favorites_json` / `CatalogRow::name` JSON shapes stay byte-stable.
