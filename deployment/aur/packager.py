#!/usr/bin/env python3
"""Generate PKGBUILD files for the llamastash AUR packages.

Usage:
    packager.py llamastash <version> <template_path> <output_path> \\
        <sha_source>

    packager.py llamastash-bin <version> <template_path> <output_path> \\
        <sha_x86_64_linux_gnu> <sha_aarch64_linux_gnu>

The llamastash-git package is static -- it has no version or sha
...
def render_source(
    ...
    """Render the llamastash (source) PKGBUILD. Returns the rendered text."""
...
def render_bin(
    ...
    """Render the llamastash-bin PKGBUILD. Returns the rendered text."""
    version = _normalize_version(version)
...
def main(argv: list[str]) -> None:
    if len(argv) < 2:
        ...
        _die("missing subcommand (expected `llamastash` or `llamastash-bin`)")

    sub = argv[1]
    if sub not in _EXPECTED_ARGC:
        _die(f"unknown subcommand: {sub!r} (expected `llamastash` or `llamastash-bin`)")
    ...
    if sub == "llamastash":
        ...
        print("Generating PKGBUILD (llamastash, source)")
        ...
        print("Generating PKGBUILD (llamastash-bin)")
        print(f"     VERSION: {version}")
        print(f"     TEMPLATE PATH: {template_path}")
        print(f"     SAVING AT: {output_path}")
