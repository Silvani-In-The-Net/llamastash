...
SOURCE_TEMPLATE = str(HERE / "llamastash" / "PKGBUILD.template")
BIN_TEMPLATE = str(HERE / "llamastash-bin" / "PKGBUILD.template")
...
class TestArgvHandling(_RenderTestBase):
    ...
    def test_unknown_subcommand_exits_2(self):
        with self.assertRaises(SystemExit) as cm:
            packager.main(["packager.py", "llamastash-git", "0.0.1"])
        self.assertEqual(cm.exception.code, 2)

    def test_source_wrong_argc_exits_2(self):
        with self.assertRaises(SystemExit) as cm:
            packager.main(["packager.py", "llamastash", "0.0.1"])
        self.assertEqual(cm.exception.code, 2)

    def test_bin_wrong_argc_exits_2(self):
        with self.assertRaises(SystemExit) as cm:
            packager.main(["packager.py", "llamastash-bin", "0.0.1", SOURCE_TEMPLATE])
        self.assertEqual(cm.exception.code, 2)
