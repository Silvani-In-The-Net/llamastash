...
pub struct PresetBody {
  ...
  /// descriptor id. Parallel to `knobs`; empty for every shipping backend, so
...
mod tests {
  ...
  fn preset_body_carries_backend_knobs_round_trip() {
    ...
    let body: PresetBody =
      yaml_serde::from_str("ctx: 8192\nbackend_knobs:\n  kv_bits: \"8\"\n  trust_remote: auto\n")
        .unwrap();
    assert_eq!(body.knobs.ctx, Some(KnobValue::Set(8192)));
    assert_eq!(
      body.backend_knobs.get("kv_bits"),
      Some(&KnobValue::Set("8".to_string()))
    );
    assert_eq!(
      body.backend_knobs.get("trust_remote"),
      Some(&KnobValue::Auto)
    );
